...
from .layers import transformation_from_parameters, disp_to_depth, compute_rigid_flow, flow_warp, image_similarity, L2_norm
import tqdm
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_teacher:
            ...
            mono_encoder = networks.ResnetEncoder(num_layers=opt.num_layers, pretrained=False)
            ...
            mono_decoder = networks.DepthDecoder(mono_encoder.num_ch_enc)
            mono_decoder_dict = torch.load(mono_decoder_path)
            mono_decoder.load_state_dict(mono_decoder_dict, strict=True)
            mono_decoder.cuda()
            mono_decoder.eval()

            flow_path = os.path.join(opt.load_weights_folder, "flow.pth")
            flow_net = networks.FlowNet(12, 0.1)
            flow_net_dict = torch.load(flow_path)
            flow_net.load_state_dict(flow_net_dict, strict=True)
            flow_net.cuda()
            flow_net.eval()
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                if opt.eval_teacher:
                    ...
                    feats = mono_encoder(input_color)
                    out = mono_decoder(feats)
                    data[('color_aug', -1, 0)] = data[('color_aug', -1, 0)].cuda()
                    data[('color_aug', 0, 0)] = data[('color_aug', 0, 0)].cuda()
                    _, depth = disp_to_depth(out[("disp", 0)], min_depth_bin, max_depth_bin)
                    depth = depth.squeeze(1)
                    pred_mono_depth = depth
                    rigid_flow = compute_rigid_flow(relative_poses, depth,
                                                    data['K', 0][:, :3, :3], reverse_pose=False)
                    warped_tgt_image = flow_warp(data[('color_aug', -1, 0)], rigid_flow)
                    rigid_error = image_similarity(input_color, warped_tgt_image)

                    fw_flownet_input = torch.cat((data[('color_aug', -1, 0)],
                                                  data[('color_aug', 0, 0)],
                                                  warped_tgt_image,
                                                  rigid_flow,
                                                  L2_norm(rigid_error, dim=1)), dim=1)

                    resflow = flow_net(fw_flownet_input)

                    outputs = {}
                    for i in range(4):
                        outputs[("resflow", i, -1)] = resflow[i]
                    ...
                    output, lowest_cost, _ = encoder(input_color, lookup_frames,
                                                           relative_poses,
                                                           K,
                                                           invK,
                                                           flow,
    ...
    for i in tqdm.tqdm(range(pred_disps.shape[0])):
        ...
        data['doj_mask'] = F.interpolate(data['doj_mask'], [gt_height, gt_width])
        doj_mask = data['doj_mask'][0][0]
        doj_mask = doj_mask[256:, 192:1856].bool()
        doj_mask = doj_mask.numpy()
