...
class MonoDataset(data.Dataset):
    ...
    def preprocess(self, inputs, color_aug):
        ...
        if self.mask_noise:
            inputs["doj_mask"] = self.resize[0](inputs["doj_mask"]);
            inputs["doj_mask"] = self.to_tensor(inputs["doj_mask"])
            if type(self).__name__ in ["CityscapesPreprocessedDataset", "CityscapesEvalDataset", "KITTIRAWDataset"]:
                inputs["doj_mask-1"] = self.resize[0](inputs["doj_mask-1"]);
                inputs["doj_mask-1"] = self.to_tensor(inputs["doj_mask-1"])
                inputs["doj_mask+1"] = self.resize[0](inputs["doj_mask+1"]);
                inputs["doj_mask+1"] = self.to_tensor(inputs["doj_mask+1"])
